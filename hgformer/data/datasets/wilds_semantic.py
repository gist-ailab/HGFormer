...
def _get_wilds_files(dataset_dir, ann_files):
    files = []
    ...
    for ann_file in ann_files:
        ...
        for line in lines:
            if '.png' not in line:  # png 확장자가 없는 라인은 무시합니다.
                continue
            line = line.strip()
            image_path, label_path = line.split(',')[1], line.split(',')[2]
            
            # 파일 경로에 대해 완전한 경로를 생성합니다.
            image_file = os.path.join(dataset_dir, image_path)
            label_file = os.path.join(dataset_dir, label_path.replace('indexLabel', 'dgss_id'))
...
def load_wilds_semantic(
    ann_files=['splits/opt2d/train.csv', 'splits/opt2d/val.csv'],
    dataset_dir='/home/jovyan/SSDb/seongju_lee/dset/WildScenes'
):
    """
    Args:
    ...
    for image_file, label_file in _get_wilds_files(dataset_dir, ann_files):
